...
**Goal:** 나이트메어 N3~N6의 난도 증가 폭을 완화하고, 실제 게임 로직을 사용하는 혼합 타워 시뮬레이션으로 N2~N6의 2별 이상 클리어 가능성을 회귀 검증한다.

**Architecture:** 기존 `NIGHTMARE_DIFFICULTY` 데이터만 조정하고 런타임 타입이나 분기를 추가하지 않는다. 테스트는 스테이지별 건설 가능 타일을 경로 커버리지 순으로 선택해 화살 2개와 슬로우 1개를 먼저 배치한 뒤 실제 `updateGame` 루프를 실행한다.
...
- 몬스터·타워 개별 수치, 웨이브 구성, 시작 골드 280G, 창고 HP 12, 보상 배율 0.85와 별점 기준은 변경하지 않는다.
...
- Produces: N3~N6 승인 배율과 `nightmare-2`~`nightmare-6` 혼합 타워 2별 이상 회귀 계약
...
const expected = [
  ...
  ['잿빛 폐허', 25, 52, 1.07, 1.01, 1.00, 1.00, 19_000, 23_500],
  ['핏빛 협곡', 26, 54, 1.13, 1.02, 0.98, 1.02, 19_500, 24_000],
  ['흑요석 광산', 24, 50, 1.21, 1.03, 0.96, 1.05, 19_500, 24_000],
  ['심연의 성문', 23, 48, 1.30, 1.05, 0.94, 1.08, 20_500, 25_000],
] as const;
```

- [ ] **Step 2: N2~N6 공통 혼합 타워 빌드 순서와 클리어 테스트를 작성한다**

`tests/game/nightmareBalance.test.ts`에 `StageNumber`를 가져오고 기존 N2
전용 헬퍼를 다음 공통 헬퍼로 교체한다.
...
function pathCoverage(stageKey: NightmareStageKey, build: Build): number {
...
function mixedBuildOrder(stageKey: NightmareStageKey): readonly Build[] {
  const occupied = new Set<string>();
  const openingArrows = rankedBuilds(stageKey, 'arrow', occupied).slice(0, 2);
  if (openingArrows.length !== 2) throw new Error(`${stageKey} needs two arrow cells`);
  openingArrows.forEach((build) => occupied.add(cellKey(build)));

  const slow = rankedBuilds(stageKey, 'slow', occupied)[0];
  if (slow === undefined) throw new Error(`${stageKey} needs one slow cell`);
...
it.each([
  ...
  'nightmare-5',
  'nightmare-6',
] satisfies readonly NightmareStageKey[])(
...
`1.10/1.02/0.98/1.02`와 승인 값
`1.07/1.01/1.00/1.00` 차이로 실패한다. N3~N6 클리어 테스트 중 현재
난도에서 통과하지 못하는 스테이지가 함께 표시되면 그 결과도 RED 증거로
기록한다.

- [ ] **Step 4: 승인된 N3~N6 배율을 적용한다**

`src/game/stages/stageCatalog.ts`의 N3~N6 항목만 다음처럼 변경한다.
...
Expected: 두 파일의 모든 테스트가 통과하며 N2~N6의 각 시뮬레이션 결과가
`victory`, 별점 `>= 2`, 슬로우 타워 배치 `true`다.
...
- Produces: 화살-only 1별 가능성과 혼합 구성 2~3별 안정성의 실제 플레이 기록, 미완료 백로그 0개
...
Expected: 숙련된 경로 커버리지 배치로 최소 1별 클리어가 가능하거나, 실패할
경우 창고 잔여 HP와 도달 웨이브를 기록해 항목을 완료 처리하지 않는다.

- [ ] **Step 2: 공개 게임에서 슬로우 혼합 구성을 확인한다**

같은 스테이지에서 시작 골드로 화살 2개와 슬로우 1개를 배치하고 이후 화살
타워를 추가한다.
...
```md
- [x] 배포 후 화살-only 1별 가능 여부와 슬로우 혼합 구성의 2~3별 안정성을 실제 플레이로 확인
```
