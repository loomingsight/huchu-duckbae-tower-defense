...
import { DEFAULT_STAGE_KEY } from '../stages/stageIdentity';
import { isWithinRadius } from './radius';

export function enemyPosition(
  enemy: GameEnemy,
  stageKey: unknown = DEFAULT_STAGE_KEY,
): Vec2 | undefined {
  if (!Number.isFinite(enemy.progress)) return undefined;

  const pathCells = getStageDefinition(stageKey).map.pathCells;
  const lastIndex = pathCells.length - 1;
  ...
  const start = cellCenter(pathCells[startIndex]);
  const end = cellCenter(pathCells[endIndex]);
...
export function selectTarget(
  ...
  for (const enemy of enemies) {
    ...
    const position = enemyPosition(enemy, stageKey);
