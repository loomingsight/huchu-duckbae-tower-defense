...
- 노멀 6개 맵은 꽃가루, 물빛, 나뭇잎, 흙먼지, 광물빛, 불씨와 조명으로 서로 다른 분위기를 냅니다.
...
- 외부 음원 없이 합성한 노멀·나이트메어 전투곡과 보스곡 4종을 재생하며, 보스 출현 시 1초 동안 자연스럽게 전환합니다.
...
6. 상단의 `정지`/`계속`, `1×`/`2×`, 소리 버튼으로 전투를 조절합니다. 소리 버튼은 효과음과 BGM을 함께 음소거합니다.
...
| `src/game/audio/` | Web Audio 효과음, 4종 절차형 BGM 데이터와 look-ahead sequencer |
...
보존합니다. 외부 오디오 파일 대신 Web Audio oscillator로 짧은 효과음과
16마디 반복 BGM 4종을 합성합니다. 일시정지나 세로 화면 차단 중에는 BGM을
30%로 낮추고, 재개 시 원래 음량으로 복원합니다.
